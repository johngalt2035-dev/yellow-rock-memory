...
```
Install the Yellow Rock Executive Assistant communications system from these two repositories:
...
```
I want to set up the Yellow Rock Executive Assistant communications system.
...
Import both with: yellow-rock-memory --db ~/.yellow-rock/memory.db train style-[type].json
```
...
```
Help me install the Yellow Rock Executive Assistant communications system.
...
```
I need communication style training data for Yellow Rock Memory.
...
OpenClaw stores these as long-term memories in the Yellow Rock Memory system automatically. No JSON, no YAML, no files to edit.
